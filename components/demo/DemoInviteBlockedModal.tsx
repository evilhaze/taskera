...
const TITLE = "Приглашение участников недоступно в демо-режиме";
const BODY =
  "В демо-режиме вы можете изучить возможности платформы, но приглашение реальных пользователей недоступно.";

const EVENT_NAME = "open-demo-invite-blocked";

export function DemoInviteBlockedModal({
  open,
  onClose
}: {
  open: boolean;
...
export function openDemoInviteBlockedModal() {
  if (typeof window !== "undefined") {
    window.dispatchEvent(new CustomEvent(EVENT_NAME));
  }
}

export function useDemoInviteBlockedListener(openModal: () => void) {
  useEffect(() => {
